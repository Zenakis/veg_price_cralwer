...
class VegtableProductJson(object):
    def __init__(self, markets, day_of_transcation, updated):
        self.markets = markets
        self.day_of_transcation = day_of_transcation
        self.updated = updated

class Market(object):
    def __init__(self, id, name, vegtables):
        self.id = id
        self.name = name
        self.vegtables = vegtables

class VegtableProductPrice(object):
    def __init__(self, id, name, upper, median, lower, avg, trading):
        self.id = id
        self.name = name
        self.upper = upper
        self.median = median
...
#                    print map(remove_tags, parse_veg_data.xpath('//text()').extract())
    ...
    #確認資訊更新時間
    def check_update_time(self, veg_data, day_of_transcation):
        if os.path.exists(self.UPDATE_FILE_LOG):
            text_file = codecs.open(self.UPDATE_FILE_LOG, 'r', 'utf-8')
            if unicode(str(day_of_transcation),'utf-8') == unicode(text_file.readlines()[0].encode('utf-8'),'utf-8'):
                print "Data Uploaded"
            else:
                self.save_update_time(str(day_of_transcation))
                self.convert_vegtable_data(veg_data, day_of_transcation)
            text_file.close()
        else:
            self.save_update_time(str(day_of_transcation))
    ...
    def convert_vegtable_data(self, veg_data, day_of_transcation):
